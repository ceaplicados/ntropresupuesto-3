...
import PartidasGenericasCg from './PartidasGenericasCg';
import UnidadesResponsablesCg from './UnidadesResponsablesCg';
import './ConceptosGenerales.css';
...
const ConceptosGenerales = () => {
        ...
        if(versiones.length>0 && estadoActual.Codigo){
            ...
            if(cgActual?.Clave !== claveCG){
                const getHistoricoCG = async (claveCG) => {
                    ...
                    response?.data[0]?.conceptoGeneral ? setCgActual(response.data[0].conceptoGeneral) : null;
                }
    ...
    { <PartidasGenericasCg cgActual={cgActual} /> }
