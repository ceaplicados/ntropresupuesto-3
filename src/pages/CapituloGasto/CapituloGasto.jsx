            ...
            if(cgActual?.Clave !== claveCG){
                const getHistoricoCG = async (claveCG) => {
                    ...
                    urlVariables.get('v') ?
                        ...
                        : null; 
                    const response = await axios(estadoActual.Codigo+'/CapituloGasto/'+claveCG+'?v='+idsVersiones.join(','));
                    response?.data?.presupuestos?.length>0 ? setPresupuestoHistorico(response.data.presupuestos) : null;
                    response?.data?.capituloGasto ? setCgActual(response.data.capituloGasto) : null;
                }
