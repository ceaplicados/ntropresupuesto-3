...
const ConceptosGeneralesCG = ({cgActual}) => {
    ...
    useEffect(() => {
        ...
        const labelsActual=conceptosGeneralesDeflactado.map((concepto) => {
            return concepto.Clave+' - '+concepto.Nombre;
        });
        let tablaActual=[];
        ...
        const configChartHistorico = {
            ...
            datasets: conceptosGeneralesDeflactado.map((concepto) => {
                return {
                    label: concepto.Clave+' - '+concepto.Nombre,
                            ...
                            return (
                                <tr key={renglon.Id}>
                                    <td>
                                        ...
                                        {renglon.Clave} - {renglon.Nombre} <Link to={'./../../ConceptosGenerales/'+renglon.Clave}><span className="material-symbols-outlined">arrow_circle_right</span></Link>
                                    </td>
                                    { versiones.filter((version) => version.Actual).map((version) => {
